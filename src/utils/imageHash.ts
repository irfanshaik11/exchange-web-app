...
function md5(str: string): string {
  ...
  function rstrMD5(s: string): string {
    ...
    for (let i = 0; i < s.length * 8; i += 8) {
      bin[i >> 5] = (bin[i >> 5]! || 0) | ((s.charCodeAt(i / 8) & mask) << (i % 32));
    }
...
const hashCache = new Map<string, string>(); // normalizedUrl -> full proxy URL
...
function normalizeForHash(url: string): string {
  try {
    ...
    const qs = sorted.length > 0 ? '?' + sorted.map(([k, v]) => `${k}=${v}`).join('&') : '';
    return base + qs;
  } catch {
    return url;
  }
 ...
 *
 * Returns `/api/img/{md5}?s={base64url}` for external HTTP URLs.
 * Passes through data:, blob:, /api/ URLs unchanged.
 */
export function computeHashImageUrl(src: string | null): string | null {
  if (!src) return null;

  // Pass through URLs that don't need proxying
  if (src.startsWith('/api/') || src.startsWith('data:') || src.startsWith('blob:')) {
    return src;
  }
  ...
  const proxyUrl = `/api/img/${hash}?s=${encoded}`;

  // Store in cache with LRU eviction
  hashCache.set(normalized, proxyUrl);
