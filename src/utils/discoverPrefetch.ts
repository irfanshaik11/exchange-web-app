...
function normalizeXStocksToken(result: any): Record<string, any> {
  ...
  return {
    ...
    // Axiom-style windows mapped from nearest Codex buckets: 24h → 1m, 4h/12h → 30m.
    volume_1m: volume24h,
  ...
  try {
    ...
    const normalized = results
      .filter((r: any) => {
        ...
        if (WRAPPED_SOL_MINTS.has((t.address || "").toLowerCase()))
          return false;
...
function normalizePulseToken(token: any): Record<string, any> {
  ...
  // Volume fields
  const vol24h =
    token.volume_24h_usd ?? token.volume_24h ?? token.volume24h ?? 0;
  const vol12h =
    token.volume_12h_usd ?? token.volume_12h ?? token.volume12h ?? 0;
  ...
  // Axiom-style windows mapped onto surviving slots: 24h → 1m, 6h → 30m.
  normalized.volume_1m = toNumber(vol24h) || sumVolumes(buyVol24, sellVol24);
  normalized.volume_24h = normalized.volume_1m;
  normalized.volume_12h = toNumber(vol12h);
  normalized.volume_30m = toNumber(vol6h);
  ...
  // Price changes
  normalized.price_percent_change_1m = toNumber(
    token.price_percent_change_24h ?? token.priceChange24h ?? 0,
  );
  normalized.price_percent_change_24h = normalized.price_percent_change_1m;
  normalized.price_percent_change_12h = toNumber(
    token.price_percent_change_12h ?? token.priceChange12h ?? 0,
  );
  normalized.price_percent_change_30m = toNumber(
    token.price_percent_change_6h ?? token.priceChange6h ?? 0,
  );
  normalized.price_percent_change_6h = normalized.price_percent_change_30m;
  normalized.price_percent_change_1h = toNumber(
    token.price_percent_change_1h ?? token.priceChange1h ?? 0,
  );
  normalized.price_percent_change_5m = toNumber(
    token.price_percent_change_5m ?? token.priceChange5m ?? 0,
  );

  // Transaction counts — 24h → 1m slot for parity with the new windows.
  normalized.total_buys_1m = toNumber(
    token.total_buys_24h ?? token.buys24h ?? 0,
  );
  normalized.total_sells_1m = toNumber(
    token.total_sells_24h ?? token.sells24h ?? 0,
  );
  normalized.total_buys_24h = normalized.total_buys_1m;
  normalized.total_sells_24h = normalized.total_sells_1m;
  normalized.total_buys_1h = toNumber(token.total_buys_1h ?? token.buys1h ?? 0);
  normalized.total_sells_1h = toNumber(
    token.total_sells_1h ?? token.sells1h ?? 0,
  );
  normalized.total_buys_5m = toNumber(token.total_buys_5m ?? token.buys5m ?? 0);
  normalized.total_sells_5m = toNumber(
    token.total_sells_5m ?? token.sells5m ?? 0,
  );

  // Price
  normalized.price_usd = toNumber(
    token.price_usd ?? token.priceUsd ?? token.price ?? 0,
  );
