...
export function useMetaMaskWallet(): UseMetaMaskWalletReturn {
  ...
  const connect = useCallback(async (): Promise<boolean> => {
    ...
    try {
      console.log('Attempting to connect to MetaMask wallet...');
      ...
      const connectPromise = metamaskProvider.request({ method: 'eth_requestAccounts' });
      ...
      console.log('MetaMask wallet connected successfully:', address);
      ...
      if (error.code === 4001) {
        ...
        errorMessage = 'Please check MetaMask - request is pending';
      } else if (error.message?.includes('timed out')) {
        errorMessage = 'Connection request timed out. Please check MetaMask and try again.';
  ...
  const signMessage = useCallback(async (message: string): Promise<{ address: string; signature: string; message: string } | { error: string }> => {
    ...
    try {
      console.log('Attempting to sign message with MetaMask wallet...');
      ...
      console.log('MetaMask signing with address:', currentAddress);
      ...
      console.log('Message signed successfully:', { address: currentAddress, message });
