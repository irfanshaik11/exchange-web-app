...
export default async function handler(req: NextApiRequest, res: NextApiResponse) {
  ...
  try {
    ...
    console.log('[pulse-new-monad] Using service URL:', monadServiceUrl);
    console.log('[pulse-new-monad] ENV vars:', {
    ...
    const fetchUrl = `${monadServiceUrl}/v1/pulse/new?${params.toString()}`;
    console.log('[pulse-new-monad] Fetching from:', fetchUrl);

    // Fetch from Monad backend service
    const upstream = await fetchWithTimeout(fetchUrl);
    const text = await upstream.text();

    if (!upstream.ok) {
      console.error('[pulse-new-monad] Upstream error:', upstream.status, text);
