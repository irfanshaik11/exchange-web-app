...
export default async function handler(
  ...
  try {
    ...
    const validTimeframes = ["1m", "5m", "30m", "1h"];
    const timeframe =
      requestedTimeframe && validTimeframes.includes(requestedTimeframe)
        ? requestedTimeframe
        : "1h"; // fallback to 1h if invalid/missing

    if (requestedTimeframe !== timeframe) {
      console.warn(
        "[pulse-trending] Invalid timeframe requested:",
        requestedTimeframe,
        "using:",
        timeframe,
      );
        ...
        const mapped = raw.map((r: any, index: number) => {
          ...
          const estimateFrom24h = (
            ...
            if (value24h > 0) {
              switch (timeframe) {
                case "1m":
                  return value24h / 1440; // 24h / 1440 = 1m
                case "5m":
                  return value24h / 288; // 24h / 288 = 5m
                case "30m":
                  return value24h / 48; // 24h / 48 = 30m
                case "1h":
                  return value24h / 24; // 24h / 24 = 1h
                default:
                  return value24h;
              }
            }
          ...
          const isCodexFormat =
            "volume" in r || "change" in r || "buyCount" in r;
          const isLegacyFormat =
            "volume24h" in r || "priceChange24h" in r || "score" in r;
          ...
          if (isCodexFormat) {
            ...
            return {
              ...
              symbol: r.symbol || "",
              launchpad_protocol:
                r.launchpad_protocol ||
                r.launchpadProtocol ||
                r.launchpadName ||
                null,
              usd_price: parseFloat(r.priceUSD || r.price_usd || "0"),
              fully_diluted_value: parseFloat(
                r.marketCap || r.market_cap_usd || "0",
              ),
              volume_1m: timeframe === "1m" ? volumeForTimeframe : 0,
              volume_5m: timeframe === "5m" ? volumeForTimeframe : 0,
              volume_30m: timeframe === "30m" ? volumeForTimeframe : 0,
              volume_1h: timeframe === "1h" ? volumeForTimeframe : 0,
              total_liquidity_usd: parseFloat(
                r.liquidity || r.total_liquidity_usd || "0",
              ),
              ...
              total_buys_1m: timeframe === "1m" ? totalBuys : 0,
              total_buys_5m: timeframe === "5m" ? totalBuys : 0,
              total_buys_30m: timeframe === "30m" ? totalBuys : 0,
              total_buys_1h: timeframe === "1h" ? totalBuys : 0,
              total_sells_1m: timeframe === "1m" ? totalSells : 0,
              total_sells_5m: timeframe === "5m" ? totalSells : 0,
              total_sells_30m: timeframe === "30m" ? totalSells : 0,
          ...
          if (isLegacyFormat) {
            ...
            return {
              ...
              usd_price: parseFloat(r.priceUsd || r.price_usd || "0"),
              fully_diluted_value: parseFloat(
                r.marketCapUsd || r.market_cap_usd || "0",
              ),
              // Legacy backend only exposes 24h; slot it into the surviving 1m field.
              volume_1m: volume24h,
              volume_5m: 0,
              volume_30m: 0,
              volume_1h: 0,
          ...
          return {
            ...
            launchpad_protocol:
              ...
              r.launchpadName ||
              null,
            usd_price: parseFloat(
              r.priceUSD || r.priceUsd || r.price_usd || "0",
            ),
            fully_diluted_value: parseFloat(
              r.marketCap || r.marketCapUsd || r.market_cap_usd || "0",
            ),
            // Unknown backend shape: slot any 24h-style volume into the 1m field.
            volume_1m: parseFloat(r.volume24h || r.volume || "0"),
            volume_5m: 0,
            volume_30m: 0,
            volume_1h: 0,
            total_liquidity_usd: parseFloat(
