...
export default async function handler(req: NextApiRequest, res: NextApiResponse) {
  ...
  try {
    ...
    const upstream = await fetchWithTimeout(`${monadServiceUrl}/v1/pulse/migrated?${params.toString()}`);
    const text = await upstream.text();

    if (!upstream.ok) {
      console.error('[pulse-migrated-monad] Upstream error:', upstream.status, text);
