...
export function InsightPanel({ source, marketId, docked = false }: InsightPanelProps) {
    ...
    <>
      {isLoading ? (
        ...
        <div className="flex flex-col items-center justify-center py-10 text-center">
          ...
          <p className="text-[11px] text-zinc-500 leading-relaxed max-w-[200px]">AI insights are being generated for this market</p>
      ...
      <div role="region" aria-label="AI Insights">
        ...
        >
          <div className="flex items-center gap-2">
            <AIIcon size={14} />
            <span className="text-[11px] font-bold tracking-[0.12em] uppercase text-[#4ADE80]">AI Insights</span>
          </div>
          <HiChevronDown className={`w-3.5 h-3.5 text-zinc-500 transition-transform ${collapsed ? '' : 'rotate-180'}`} />
        </button>
        {!collapsed && (
          <>
  ...
  // ── Floating mode: mobile button + modal, desktop draggable panel ──
  return (
    <>
        ...
        <div className="flex-1 overflow-y-auto p-4 pt-24" onClick={(e) => e.stopPropagation()}>
          <div className="iridescent-border max-w-md mx-auto">
            <div className="iridescent-inner overflow-hidden">
              ...
              <div className="h-[1px] w-full" style={{ background: 'linear-gradient(90deg, transparent, rgba(74,222,128,0.15) 50%, transparent)' }} />
              <div className="p-3 space-y-1.5">{insightContent}</div>
            </div>
          </div>
        </div>
      </div>
      ...
      {collapsed ? (
        ...
        <div className="iridescent-border w-full">
          <div className="iridescent-inner overflow-hidden flex flex-col" style={{ maxHeight: 'calc(100vh - 8rem)' }}>
            <button onClick={() => setCollapsed(true)} className="flex items-center justify-between px-3 py-3 hover:bg-white/[0.02] transition-colors w-full">
              <div className="flex items-center gap-2.5">
                <AIIcon size={16} />
                <span className="text-[13px] font-bold tracking-[0.12em] uppercase text-[#4ADE80]">AI Insights</span>
              </div>
              <HiChevronDown className="w-4 h-4 text-zinc-500 rotate-180" />
            </button>
            <div className="h-[1px] w-full flex-shrink-0" style={{ background: 'linear-gradient(90deg, transparent, rgba(74,222,128,0.15) 50%, transparent)' }} />
            <div className="flex-1 overflow-y-auto p-2.5 space-y-1.5">{insightContent}</div>
          </div>
        </div>
      )}
    </motion.div>
