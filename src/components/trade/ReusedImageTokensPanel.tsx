// components/right/ReusedImageTokensPanel.tsx
...
interface Props {
  tokens: ReusedToken[];
  className?: string;
  title?: string;        // default: "Reused Image Tokens (O)"
  loading?: boolean;
  emptyText?: string;
  ...
  persistKey?: string;   // default: "ReusedImageTokensPanel.collapsed"
}
...
const Row: React.FC<{ t: ReusedToken }> = ({ t }) => {
...
const Header: React.FC<{
  ...
  return (
    <div className={`sticky top-0 z-10 flex items-center justify-between px-3 py-2 border-b ${AX.border} bg-[#151719]/95 backdrop-blur`}>
      <button
        onClick={onToggleCollapse}
        className="flex items-center gap-2 group/btn"
        aria-expanded={!collapsed}
        aria-controls="reused-panel-body"
...
const ReusedImageTokensPanel: React.FC<Props> = ({
  tokens,
  className,
  ...
  persistKey = "ReusedImageTokensPanel.collapsed",
}) => {
  ...
  return (
    ...
    >
      ...
      {/* Collapsible content */}
      <div id="reused-panel-body" style={wrapperStyle}>
        {!collapsed && (
...
export default ReusedImageTokensPanel;
