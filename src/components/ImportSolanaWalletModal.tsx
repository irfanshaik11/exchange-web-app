...
interface ImportSolanaWalletModalProps {
  isOpen: boolean;
  onClose: () => void;
  onImported?: () => Promise<void> | void;
}

export default function ImportSolanaWalletModal({
  isOpen,
  onClose,
  onImported,
}: ImportSolanaWalletModalProps) {
  ...
  const resetState = useCallback(() => {
    ...
    const existingIframe = document.getElementById("turnkey-import-solana-iframe");
    if (existingIframe) {
    ...
    const setupIframe = async () => {
      ...
      try {
        ...
        const container =
          ...
          document.getElementById("turnkey-import-solana-iframe-container");

        const stamper = new IframeStamper({
          iframeUrl: "https://import.turnkey.com",
          iframeContainer: container || undefined,
          iframeElementId: "turnkey-import-solana-iframe",
        ...
        setIframeReady(true);
        setStatus("Enter your Solana recovery phrase in the secure field below.");
      } catch (err: any) {
        if (cancelled) return;
  ...
  const handleImport = async () => {
    ...
    try {
      ...
      if (importMode === "privateKey") {
        // Non-iframe method for Solana private keys (accepts base58 directly)
        ...
        body = {
          encryptedBundle,
          keyFormat: "solana",
          privateKeyName: "Imported Solana Private Key",
        };
      } else {
      ...
      toast.success("Solana wallet imported securely");
      if (onImported) {
        await onImported();
      }
      try {
    ...
    >
      ...
      >
          ...
          <div className="flex items-center justify-between">
            <div>
              ...
              <p className="text-xs text-[#9CA3AF]">
                Secure Turnkey iframe import for Solana (SOL)
              </p>
            </div>
            <button
        ...
        <div className="px-6 py-5 space-y-4">
          ...
          {importMode === "privateKey" ? (
            <div className="p-3 rounded-lg bg-[#0b0c0f] border border-[#2A2B33]">
              <div className="text-xs text-[#9CA3AF] mb-2">
                Paste your Solana private key below (base58 format)
              </div>
              <textarea
                value={privateKeyInput}
                onChange={(e) => setPrivateKeyInput(e.target.value)}
                placeholder="Paste your Solana base58 private key here (e.g., 2rdbdu9CqYh...)"
                disabled={isImporting}
            ...
            <div>
              ...
              <div
                id="turnkey-import-solana-iframe-container"
                ref={containerRef}
                className="w-full"
                style={{ minHeight: "200px", minWidth: "600px" }}
              >
                {!iframeReady && (
                  <div className="rounded-lg bg-[#111319] border border-dashed border-[#2A2B33] flex items-center justify-center text-sm text-[#6B7280]" style={{ height: "200px", width: "600px" }}>
          ...
          >
            {isImporting ? "Importing…" : "Import Solana Wallet"}
          </button>
