...
export default function DiscoverContent() {
    ...
    const fetchNewPairs = async (
      ...
      try {
        const response = await fetch(
          `/api/token-service/pulse-new?limit=200&fresh=1`,
          {
        ...
        if (Array.isArray(payload)) {
          ...
          const normalized = payload
            ...
            .filter((t) => t && t.mint && !isWrappedSol(t));
          if (cancelled) return;
  ...
  useEffect(() => {
    ...
    const normalizeXStocksToken = (result: any): TokenWithDexPaid => {
      ...
      const normalized: Record<string, any> = {
        ...
        // Axiom-style windows (1m/5m/30m/1h) mapped from nearest Codex buckets:
        ...
        total_buys_1m: toNumber(result.buyCount24 || "0"),
        total_buys_1h: toNumber(result.buyCount1 || "0"),
        total_buys_30m: toNumber(result.buyCount4 || "0"),
        ...
        total_sells_1m: toNumber(result.sellCount24 || "0"),
        total_sells_1h: toNumber(result.sellCount1 || "0"),
        total_sells_30m: toNumber(result.sellCount4 || "0"),
        ...
        txnCount30m: toNumber(result.txnCount4 || "0"),
    ...
    const fetchXStocks = async (useCache = true, showLoading = false) => {
            ...
            if (cachedData) {
              ...
              if (age > CACHE_TTL) {
                fetchXStocks(false, false).catch((err) => {
                  console.error(
                    "[DiscoverContent] xStocks background refresh failed:",
                    err,
                  );
                });
              }
      ...
      try {
        ...
        if (cancelled) return;
        const message =
          err instanceof Error ? err.message : "Failed to fetch xStocks";
    ...
    if (hasInitialData) {
      ...
      fetchXStocks(false, false).catch((err) => {
        console.error(
          "[DiscoverContent] xStocks background fetch failed:",
          err,
        );
      });
    } else {
  ...
  const applyFilters = useCallback(
    (tokens: TokenWithDexPaid[]) => {
      ...
      if (localFilters.amms && localFilters.amms.length > 0) {
        const protocolPatterns = localFilters.amms.flatMap((ammId) =>
      ...
      if (localFilters.searchKeywords.trim()) {
        const searchTerms = localFilters.searchKeywords
          .toLowerCase()
          .split(",")
          .map((term) => term.trim())
          .filter((term) => term);
      ...
      if (localFilters.excludeKeywords.trim()) {
        const excludeTerms = localFilters.excludeKeywords
          .toLowerCase()
          .split(",")
          .map((term) => term.trim())
          .filter((term) => term);
      ...
      if (localFilters.marketCapMin || localFilters.marketCapMax) {
        filtered = filtered.filter((token) => {
          ...
          const min = localFilters.marketCapMin
            ? Number(localFilters.marketCapMin)
            : 0;
          const max = localFilters.marketCapMax
            ? Number(localFilters.marketCapMax)
            : Infinity;
      ...
      if (localFilters.volumeMin || localFilters.volumeMax) {
        filtered = filtered.filter((token) => {
          ...
          const min = localFilters.volumeMin
            ? Number(localFilters.volumeMin)
            : 0;
          const max = localFilters.volumeMax
            ? Number(localFilters.volumeMax)
            : Infinity;
      ...
      if (localFilters.liquidityMin || localFilters.liquidityMax) {
        filtered = filtered.filter((token) => {
          ...
          const min = localFilters.liquidityMin
            ? Number(localFilters.liquidityMin)
            : 0;
          const max = localFilters.liquidityMax
            ? Number(localFilters.liquidityMax)
            : Infinity;
  ...
  const activeFilterCount = useMemo(() => {
    let count = 0;
    if (localFilters.amms && localFilters.amms.length > 0)
      count += localFilters.amms.length;
  ...
  const processedNewPairs = useMemo(() => {
    ...
    const base = newPairsRaw.filter((token) => {
      ...
      const protocol = (
        (token as any).launchpad_protocol ||
        (token as any).launchpadProtocol ||
      ...
      const hasVolume = ["1m", "5m", "30m", "1h"].some((tf) => {
  ...
  const toPumpPortalItem = (t: any): PumpItem => {
    ...
    const mc =
      mcNum > 0
        ? mcNum >= 1_000_000
          ? `$${(mcNum / 1_000_000).toFixed(1)}M`
          : mcNum >= 1_000
            ? `$${(mcNum / 1_000).toFixed(1)}K`
            : `$${mcNum.toFixed(0)}`
        : undefined;
  ...
  return (
    <div className="flex h-full flex-col overflow-hidden bg-[#111214] text-[#E6E7EA]">
      {/* Tab Navigation */}
      <div className="flex flex-shrink-0 flex-col gap-4 border-b border-neutral-800/60 px-4 py-4 sm:px-6 lg:flex-row lg:items-center lg:justify-between lg:gap-6">
        <div className="scrollbar-hide flex items-center gap-3 overflow-x-auto pb-2 sm:gap-4 lg:gap-6 lg:pb-0">
          <button
          ...
          */}
          <button
            className={`text-sm font-light whitespace-nowrap transition-colors sm:text-base lg:text-lg ${activeTab === "surge" ? "text-[#f0f5f5]" : "text-[#6B7280] hover:text-[#f0f5f5]"} cursor-pointer`}
            onClick={() => setActiveTab("surge")}
          >
            Surge
          </button>
          <button
            className={`text-sm font-light whitespace-nowrap transition-colors sm:text-base lg:text-lg ${activeTab === "live" ? "text-[#f0f5f5]" : "text-[#6B7280] hover:text-[#f0f5f5]"} cursor-pointer`}
              ...
              >
                {(["1m", "5m", "30m", "1h"] as Timeframe[]).map(
                  (tf: Timeframe) => (
                    <button
                      key={tf}
                      className="flex cursor-pointer items-center justify-center rounded px-1 text-sm font-medium whitespace-nowrap transition-all duration-200"
                      style={{
                        paddingTop: "2px",
                        paddingBottom: "2px",
                        backgroundColor:
          ...
          >
            {["P1", "P2", "P3"].map((pill) => {
              ...
              return (
                ...
                >
                  <button
                    className="flex cursor-pointer items-center justify-center rounded px-1 text-sm font-medium transition-all duration-200"
                    style={{
                      paddingTop: "2px",
                      paddingBottom: "2px",
                      backgroundColor:
          ...
          pumpPortalTokens.length > 0 ? (
                  ...
                  preloadTradeChart(
                    {
                      ...
                      pairAddress:
                        rawToken.pair_address || rawToken.bondingCurveKey,
                      chain: "sol",
                      name: rawToken.name,
                      ...
                      marketCapUsd:
                        ...
                        (rawToken.marketCapSol
                          ? rawToken.marketCapSol * 170
                          : undefined),
                    },
                    { router },
                  );
                ...
                if (rawToken) {
                  fetch("/api/token-service/backfill-token", {
                    ...
                    body: JSON.stringify({
                      ...
                      uri: rawToken.uri,
                      market_cap_usd:
                        rawToken.market_cap_usd || rawToken.marketCapSol
                          ? rawToken.marketCapSol * 170
                          : undefined,
            ...
            <div className="space-y-4">
              {Array.from({ length: 6 }).map((_, i) => (
                <div
                  key={i}
                  className="h-20 w-full animate-pulse rounded bg-[#1E1F26]"
                />
              ))}
            </div>
          )
