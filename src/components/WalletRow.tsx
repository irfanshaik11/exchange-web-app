...
export default function WalletRow({ wallet, onRemove }: WalletRowProps) {
  ...
  return (
    <tr key={wallet.address} className="border-b border-neutral-800/50 hover:bg-neutral-800/30 transition-all duration-300">
      <td className="w-32 px-2 py-3 text-neutral-400 text-xs">{formatDate(wallet.createdAt)}</td>
      <td className="flex-1 px-2 py-3 font-mono text-neutral-200 text-xs flex items-center gap-2">
        <span className="text-lg mr-1">💰</span><span className="truncate max-w-[calc(50%-10px)]">{wallet.name || 'N/A'}</span>
        <span className="text-neutral-400">|</span>
        <div className="flex items-center gap-1">
          <span className="truncate max-w-[calc(50%-10px)]">{wallet.address}</span>
          <button 
            className="text-neutral-500 hover:text-white transition-colors duration-300 flex-shrink-0" 
      ...
      <td className="px-2 py-3 text-right flex items-center gap-3 justify-end">
        {loading ? (
          <span className="text-neutral-500 text-xs animate-pulse">...</span>
        ) : (
          <span className="text-emerald-400 text-xs font-medium">${balance?.toFixed(2)}</span>
        )}
